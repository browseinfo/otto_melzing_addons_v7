...
class report_account_balance_sheet(report_sxw.rml_parse, common_report_header):

    def __init__(self, cr, uid, name, context=None):
        super(report_account_balance_sheet, self).__init__(cr, uid, name, context=context)
    ...
    def get_lines(self, data):
        ...
        print '_________ids3______',ids3
        acc_id = acc2_id = []
        for report in financial_report_obj.browse(self.cr, self.uid, ids3, context=data['form']['used_context']):
            ...
            for report in financial_report_obj.browse(self.cr, self.uid, [report.id], context=data['form']['used_context']):
                ...
                lines.append(valss2)
        print '_______lines_________________',lines
        return lines

report_sxw.report_sxw('report.account.financial.report.bs', 'account.financial.report',
    'addons/balance_sheet_extended/report/account_financial_report.rml', parser=report_account_balance_sheet, header=False)
